...
- `enableImageGeneration: true` registers GPT Image generation and editing. Use the image capability included with your current GPT subscription. Availability, dimensions, and quota remain account- and service-controlled.
- `codex_connect_image_generate` generates from text alone when the optional `images` input is omitted or empty, and edits when it carries at least one image. Routing follows the resolved image list rather than the intent expressed in `mode`: the service's generation endpoint accepts an `images` field and ignores it, so a request carrying images is always sent to the edit endpoint. `mode: "edit"` with no resolvable image is an error and never falls back to generating a new image. Input images come from the conversation (`kind: "recent"`, the most recent images, or `kind: "asset"`, an exact original by `assetId`); at most 20 are accepted per request. An image that cannot be resolved fails the call instead of silently editing fewer images. `preserve` entries are folded into the edit instruction as invariants to keep. The tool accepts no `size`, `quality`, `background`, or `n`: the service silently ignores invalid values for those instead of reporting an error, and the response's echoed values are the only trustworthy record.
- `imageModelHint` is an optional profile-scoped setting in Plugin configuration or the profile config. Empty uses `gpt-image-2`; a custom value accepts 1–128 ASCII letters, digits, dots, underscores, or hyphens and must start with a letter or digit. Saving changes the `model` field of subsequent image requests to the same fixed endpoint; clearing restores the default. This is an unverified route hint: the service may ignore or reject it, and it does not guarantee the returned model.
...
| `enableImageGeneration` | `false` | Register GPT Image generation and editing |
