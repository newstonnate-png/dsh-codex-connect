...
function fixture() {
  ...
  const readVersion = vi.fn(async (_name: string): Promise<string | undefined> => '0.1.7-rc.2')
...
describe('capability evidence', () => {
  it('allows the latest complete host set but blocks mixed host versions before credential reads', async () => {
    ...
    f.readVersion.mockImplementation(async name => name === '@deepseek-ai/dsh-session' ? '0.1.2-rc.1' : '0.1.7-rc.2')
  ...
  it('gates network work on the complete declared host package set, including session', async () => {
    const f = fixture()
    f.readVersion.mockImplementation(async name => name === '@deepseek-ai/dsh-session' ? '0.1.0-rc.7' : '0.1.7-rc.2')
    ...
    expect(report.checks.runtime.action).toContain('0.1.7-rc.2')
