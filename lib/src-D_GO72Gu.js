...
*/
const OPENAI_CODEX_IMAGE_EDITS_URL = "https://chatgpt.com/backend-api/codex/images/edits";
...
function assertEditInputs(images) {
	if (!Array.isArray(images) || images.length === 0) throw new OpenAICodexTransportError(OPENAI_CODEX_TRANSPORT_ERROR_CODES.invalidRequest);
	if (images.length > 20) throw new OpenAICodexTransportError(OPENAI_CODEX_TRANSPORT_ERROR_CODES.invalidRequest);
	for (const image of images) {
		if (typeof image?.b64 !== "string" || image.b64.length === 0) throw new OpenAICodexTransportError(OPENAI_CODEX_TRANSPORT_ERROR_CODES.invalidRequest);
		if (typeof image.mediaType !== "string" || !/^image\/(png|jpeg|webp|gif)$/u.test(image.mediaType)) throw new OpenAICodexTransportError(OPENAI_CODEX_TRANSPORT_ERROR_CODES.invalidRequest);
	}
	return images;
}
...
var OpenAICodexTransport = class extends Service {
	...
	async generateImages(input, context) {
		const operation = () => this.request("generate", input.prompt, void 0, context);
	...
	*/
	...
	async request(operation, prompt, images, context) {
		if (typeof prompt !== "string" || prompt.trim().length === 0 || prompt.length > 32e3) throw new OpenAICodexTransportError(OPENAI_CODEX_TRANSPORT_ERROR_CODES.invalidRequest);
		...
		if (isAborted(context.signal)) throw new OpenAICodexTransportError(OPENAI_CODEX_TRANSPORT_ERROR_CODES.canceled);
		const body = operation === "edit" ? JSON.stringify({
			model: imageModelHint,
			prompt,
			images: (images ?? []).map((image) => ({ image_url: `data:${image.mediaType};base64,${image.b64}` }))
		}) : JSON.stringify({
			model: imageModelHint,
			prompt
		});
		if (body.length > 201326592) throw new OpenAICodexTransportError(OPENAI_CODEX_TRANSPORT_ERROR_CODES.invalidRequest);
		...
		try {
			...
			return {
				...
				operation,
				...parsed.size === void 0 ? {} : { size: parsed.size },
...
const SUPPORTED_DSH_PLUGIN_API_VERSION = "0.1.6-alpha.1";
...
*/
const VERIFIED_PI_AI_BY_DSH = /* @__PURE__ */ new Map([["0.1.6-alpha.1", ["0.85.1"]]]);
...
function evaluateCompatibility(input = {}) {
	...
	const matchedPair = dshVersion === packages["@deepseek-ai/dsh-llm-pi-ai"].installed && (VERIFIED_PI_AI_BY_DSH.get(dshVersion ?? "") ?? []).includes(piVersion ?? "");
	return {
		schemaVersion: 1,
...
function positiveSafeInteger$4(value) {
...
function imageRef(value) {
	...
	const candidate = value;
	if (typeof candidate.attachmentId !== "string" || candidate.attachmentId.length === 0 || !mediaType$1(candidate.mediaType) || !positiveSafeInteger$3(candidate.bytes) || !positiveSafeInteger$3(candidate.width) || !positiveSafeInteger$3(candidate.height) || candidate.name !== void 0 && (typeof candidate.name !== "string" || candidate.name.length === 0)) return void 0;
	return {
...
function operationOf(value) {
	return value === "generate" || value === "edit" ? value : void 0;
}
...
function isSupportedMediaType(value) {
	return typeof value === "string" && SUPPORTED_MEDIA_TYPES.includes(value);
}
...
* failure later.
*/
function imageRefFromBlock(value) {
	const record = asRecord(value);
	if (record === void 0 || record["type"] !== "image") return void 0;
	...
	if (!isSupportedMediaType(ref["mediaType"])) return void 0;
	if (!positiveSafeInteger$2(ref["bytes"]) || !positiveSafeInteger$2(ref["width"]) || !positiveSafeInteger$2(ref["height"])) return void 0;
...
* for an image.
*/
function imageRefsInEvent(event) {
	const eventRecord = asRecord(event);
	if (eventRecord === void 0) return [];
...
*/
function recentImageRefs(events, count) {
...
function outputContent(value) {
	...
	return [{
		type: "text",
		text: `${value.operation === "edit" ? "Edited" : "Generated"} ${String(value.images.length)} image${value.images.length === 1 ? "" : "s"}:\n${lines.join("\n")}`
...
*/
function parseAttachmentRef(value) {
	if (typeof value !== "object" || value === null || Array.isArray(value)) failure("kind \"attachment\" requires `ref` to be an attachment reference object.");
	const record = value;
	const attachmentId = record.attachmentId;
	if (typeof attachmentId !== "string" || !attachmentId.startsWith("sha256:")) failure("kind \"attachment\" requires `ref.attachmentId` to be a sha256: attachment id.");
	const mediaType = record.mediaType;
	if (typeof mediaType !== "string" || mediaType.length === 0) failure("kind \"attachment\" requires `ref.mediaType`.");
	for (const field of [
		"bytes",
		"width",
		"height"
		...
		if (!Number.isSafeInteger(number) || number < 1) failure(`kind "attachment" requires \`ref.${field}\` to be a positive integer.`);
	}
	const name = record.name;
	const originalDimensions = record.originalDimensions;
	let dimensions;
	if (typeof originalDimensions === "object" && originalDimensions !== null) {
		const size = originalDimensions;
		if (Number.isSafeInteger(size.width) && Number.isSafeInteger(size.height) && size.width > 0 && size.height > 0) dimensions = {
			width: size.width,
			height: size.height
		};
	}
	return {
		...
		...dimensions === void 0 ? {} : { originalDimensions: dimensions }
	};
}
/**
...
*/
async function resolveInput(ctx, assets, sessionId, arg, events, signal) {
	if (arg.kind === "attachment") {
		const ref = parseAttachmentRef(arg.ref);
		if (!ctx.attachments.imageLimits.mediaTypes.includes(ref.mediaType)) failure(`${ref.mediaType} images are disabled by this deployment.`);
		if (ref.bytes > ctx.attachments.imageLimits.maxImageBytes) failure("An input image exceeds this deployment's byte limit.");
		let data;
		try {
		...
		if (data === void 0 || data.byteLength === 0) failure("The referenced attachment is empty.");
		return [{
			ref: ref.name === void 0 ? { ...ref } : {
				...ref,
				name: ref.name
			},
	...
	if (arg.kind === "asset") {
		...
		if (typeof assetId !== "string" || assetId.length === 0) failure("kind \"asset\" requires a non-empty assetId.");
		let stored;
		try {
		...
		if (stored === void 0) failure("The referenced image asset is not available in this session.");
		const { ref: original, data } = stored;
		if (!ctx.attachments.imageLimits.mediaTypes.includes(original.mediaType)) failure(`${original.mediaType} images are disabled by this deployment.`);
		if (data.byteLength > ctx.attachments.imageLimits.maxImageBytes) failure("An input image exceeds this deployment's byte limit.");
		return [{
			ref: {
				attachmentId: original.assetId,
				mediaType: original.mediaType,
				bytes: original.bytes,
				width: original.width,
				height: original.height,
				...original.name === void 0 ? {} : { name: original.name }
			},
			b64: Buffer.from(data).toString("base64")
		}];
	}
	const count = arg.count ?? RECENT_DEFAULT_COUNT;
	if (!Number.isSafeInteger(count) || count < 1 || count > 20) failure(`The recent-image count must be between 1 and ${String(20)}.`);
	if (events === void 0) failure("Recent conversation images are unavailable for this call.");
	const refs = recentImageRefs(events, count);
	if (refs.length === 0) failure("No image was found in this conversation. Attach an image or generate one first.");
	if (refs.length < count) failure(`Requested the last ${String(count)} conversation image${count === 1 ? "" : "s"}, but only ${String(refs.length)} ${refs.length === 1 ? "was" : "were"} available. Ask which image to use instead of guessing.`);
	const resolved = [];
	for (const ref of refs) {
		if (!ctx.attachments.imageLimits.mediaTypes.includes(ref.mediaType)) failure(`${ref.mediaType} images are disabled by this deployment.`);
		if (ref.bytes > ctx.attachments.imageLimits.maxImageBytes) failure("A conversation image exceeds this deployment's byte limit.");
		let data;
		try {
			data = (await ctx.attachments.readImage(ref, signal)).data;
		} catch {
		...
		if (data === void 0 || data.byteLength === 0) failure("A conversation image is empty.");
		resolved.push({
			ref,
			b64: Buffer.from(data).toString("base64")
		});
...
async function submit(ctx, transport, assets, prompt, inputs, exec) {
	const operation = inputs.length === 0 ? "generate" : "edit";
	const invoke = async () => {
		if (operation === "edit") return transport.editImages({
			prompt,
	...
	try {
		originals = await assets.saveImages(String(sessionId), uploads.map((upload, index) => {
			...
			if (parsed === void 0 || upload.name === void 0) failure("The generated image batch is incomplete.");
			return {
				data: upload.data,
				mediaType: parsed.mediaType,
				width: parsed.width,
	...
	return defineTool({
		...
		parameters: {
			prompt: {
				type: "string",
				required: true,
				description: "A complete description of the image to generate, or of the change to make when editing."
			},
			mode: {
				type: "string",
				enum: ["generate", "edit"],
			...
			images: {
				...
				items: {
					...
					properties: {
						...
						assetId: {
							type: "string",
							description: "Required when kind is \"asset\"."
						},
						ref: {
							type: "object",
						...
						count: {
							type: "integer",
							description: "How many recent conversation images to take when kind is \"recent\". Defaults to 1."
						},
						role: {
							type: "string",
							enum: [...IMAGE_ROLES],
							description: "Descriptive label only; position decides what the service acts on."
						}
					}
				}
			},
		...
		async execute(args, exec) {
			...
			for (const unsupported of [
				...
				"background",
				"n"
			]) if (Object.hasOwn(args, unsupported)) failure(`Image ${unsupported} is not supported. Output size and quality are service defaults.`);
			...
			if (requested.length > 20) failure(`This request includes ${String(requested.length)} images; at most ${String(20)} can be edited at once.`);
			const instruction = preserve.length === 0 ? prompt : `${prompt}\n\nChange only what the instruction above describes. Keep everything else unchanged, in particular: ${preserve.join("; ")}.`;
			...
			if (requested.length > 0) {
				...
				const resolved = [];
				for (const arg of requested) resolved.push(...await resolveInput(ctx, assets, String(sessionId), arg, events, exec.signal));
				if (resolved.length > 20) failure(`This request resolved to ${String(resolved.length)} images; at most ${String(20)} can be edited at once.`);
				let total = 0;
				for (const input of resolved) {
					total += input.ref.bytes;
					if (!Number.isSafeInteger(total) || total > ctx.attachments.imageLimits.maxMessageImageBytes) failure("The combined input images exceed the size limit.");
				}
				inputs = resolved;
			}
			if (inputs.length === 0 && args.mode === "edit") failure("Editing needs at least one input image. Ask which image to change rather than generating a new one.");
			const key = executionKey(exec);
			const current = inFlight.get(key);
			if (current !== void 0) return current;
			const pending = submit(ctx, transport, assets, instruction, inputs, exec).catch((error) => {
...
//#endregion
export { FastModeRegistry as $, DSH_PLUGIN_API_PACKAGES as A, isValidOpenAICodexImageModelHint as At, evaluateCompatibility as B, OPENAI_CODEX_ACCOUNT_LIMIT as Bt, checkForOpenAICodexUpdate as C, DEFAULT_OPENAI_CODEX_SEARCH_MAX_OUTPUT_TOKENS as Ct, COMPATIBILITY_CONTRACT as D, OPENAI_CODEX_SETTINGS_NAMESPACE as Dt, parseOpenAICodexVersion as E, DEFAULT_OPENAI_CODEX_SETTINGS as Et, SUPPORTED_NODE_RANGE as F, openAICodexModelCatalog as Ft, OPENAI_CODEX_LOCAL_PROXY_CANDIDATES as G, OpenAICodexCredentialStore as Gt, readInstalledPackageVersion as H, OPENAI_CODEX_AUTH_FILENAME as Ht, SUPPORTED_PI_AI_RANGE as I, loginOpenAICodex as It, OPENAI_CODEX_PROXY_PROBE_URL as J, OPENAI_CODEX_PROXY_CANDIDATE_LIMIT as K, openAICodexAuthPath as Kt, VERIFIED_PI_AI_BY_DSH as L, logoutOpenAICodex as Lt, SUPPORTED_DSH_PLUGIN_API_RANGE as M, normalizeOpenAICodexProxyUrl as Mt, SUPPORTED_DSH_PLUGIN_API_VERSION as N, resolveOpenAICodexProxyUrl as Nt, COMPATIBILITY_PACKAGES as O, decodeOpenAICodexSettings as Ot, SUPPORTED_DSH_PLUGIN_API_VERSIONS as P, resolveOpenAICodexSettings as Pt, OPENAI_CODEX_FAST_MODE_PATH as Q, assessCompatibility as R, openAICodexAuthStatus as Rt, OPENAI_CODEX_UPDATE_PATH as S, DEFAULT_OPENAI_CODEX_SEARCH_CONTEXT_SIZE as St, parseOpenAICodexUpdateResult as T, DEFAULT_OPENAI_CODEX_SEARCH_MODEL as Tt, OPENAI_CODEX_PROXY_DETECT_PATH as U, OPENAI_CODEX_AUTH_V1_BACKUP_SUFFIX as Ut, isSupportedDshPluginApiVersion as V, OPENAI_CODEX_AUTH_DOCUMENT_LIMIT as Vt, OPENAI_CODEX_PROXY_TEST_PATH as W, OPENAI_CODEX_PROVIDER as Wt, detectOpenAICodexProxies as X, OpenAICodexProxyManager as Y, listOpenAICodexProxyCandidates as Z, IMAGE_GENERATE_TOOL_NAME as _, OpenAICodexTransport as _t, name as a, OPENAI_CODEX_USAGE_URL as at, openAICodexConflictMessage as b, DEFAULT_OPENAI_CODEX_IMAGE_MODEL_HINT as bt, migrateOpenAICodexSearchHistory as c, OPENAI_CODEX_IMAGE_GENERATION_URL as ct, OPENAI_CODEX_BASE_URL as d, OPENAI_CODEX_IMAGE_MAX_RESPONSE_BYTES as dt, OPENAI_CODEX_FAST_MODE_MAX_SESSIONS as et, OPENAI_CODEX_SEARCH_PROVIDER as f, OPENAI_CODEX_IMAGE_PROMPT_MAX_LENGTH as ft, VIEW_IMAGE_TOOL_NAME as g, OPENAI_CODEX_TRANSPORT_SERVICE as gt, mapOpenAICodexSearchResponse as h, OPENAI_CODEX_TRANSPORT_ERROR_CODES as ht, inject as i, normalizeTrustedOrigin as it, PI_AI_PACKAGE as j, isValidOpenAICodexProxyUrl as jt, COMPATIBILITY_SCHEMA_VERSION as k, isValidOpenAICodexContextWindowOverrides as kt, CODEX_AUTO_REVIEW_MODEL as l, OPENAI_CODEX_IMAGE_MAX_COUNT as lt, OpenAICodexSearchProvider as m, OPENAI_CODEX_TRANSPORT_API_VERSION as mt, OPENAI_CODEX_SETTINGS_NS as n, isFastModeSessionId as nt, OPENAI_CODEX_HISTORY_BACKUP_SUFFIX as o, parseOpenAICodexUsage as ot, OPENAI_CODEX_SEARCH_URL as p, OPENAI_CODEX_IMAGE_REQUEST_TIMEOUT_MS as pt, OPENAI_CODEX_PROXY_PROBE_TIMEOUT_MS as q, apply as r, OpenAICodexTrustedOriginsStore as rt, OPENAI_CODEX_SEARCH_MODEL_REQUEST_EVENT as s, readOpenAICodexRateLimits as st, Config as t, OPENAI_CODEX_FAST_MODE_MAX_SESSION_ID_LENGTH as tt, probeCodexAutoReview as u, OPENAI_CODEX_IMAGE_MAX_ERROR_BYTES as ut, assertNoOpenAICodexProviderConflict as v, OpenAICodexTransportError as vt, compareOpenAICodexVersions as w, DEFAULT_OPENAI_CODEX_SEARCH_MODE as wt, CODEX_CONNECT_VERSION as x, DEFAULT_OPENAI_CODEX_PROXY_URL as xt, diagnoseOpenAICodex as y, isOpenAICodexTransportError as yt, detectCompatibility as z, publicAuthError as zt };
