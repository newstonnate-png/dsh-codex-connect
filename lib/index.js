...
var OpenAICodexTransport = class extends Service {
	...
	async generateImages(input, context) {
		if (this.backendRequests !== void 0) try {
			...
			}, (request) => this.request("generate", input.prompt, void 0, { signal: request.signal }, request.fetch));
		} catch (error) {
	...
	*/
	...
	async request(operation, prompt, images, context, requestFetch = globalThis.fetch) {
...
const SUPPORTED_DSH_PLUGIN_API_VERSION = "0.1.7-rc.2";
...
function parseAttachmentRef(value) {
	...
	if (originalDimensions !== void 0) {
		if (typeof originalDimensions !== "object" || originalDimensions === null || Array.isArray(originalDimensions)) failure("kind \"attachment\" requires `ref.originalDimensions` to contain positive width and height values.");
		...
		else failure("kind \"attachment\" requires `ref.originalDimensions` to contain positive width and height values.");
	}
	return {
		attachmentId,
		mediaType,
