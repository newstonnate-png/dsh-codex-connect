#!/usr/bin/env node
import { A as DSH_PLUGIN_API_PACKAGES, B as evaluateCompatibility, Ft as openAICodexModelCatalog, Gt as OpenAICodexCredentialStore, H as readInstalledPackageVersion, It as loginOpenAICodex, Kt as openAICodexAuthPath, L as VERIFIED_PI_AI_BY_DSH, Lt as logoutOpenAICodex, Mt as normalizeOpenAICodexProxyUrl, Rt as openAICodexAuthStatus, V as isSupportedDshPluginApiVersion, Wt as OPENAI_CODEX_PROVIDER, c as migrateOpenAICodexSearchHistory, d as OPENAI_CODEX_BASE_URL, it as normalizeTrustedOrigin, l as CODEX_AUTO_REVIEW_MODEL, rt as OpenAICodexTrustedOriginsStore, u as probeCodexAutoReview, x as CODEX_CONNECT_VERSION, y as diagnoseOpenAICodex, zt as publicAuthError } from "./src-D_GO72Gu.js";
...
* longer serves.
*/
function declaredPairAction(compatibility) {
	...
	return `Host and adapter must be an exact verified pair — ${detail}.${installedDsh === null && installedPiAi === null ? "" : ` Installed: DSH ${installedDsh ?? "unknown"} with pi-ai ${installedPiAi ?? "unknown"}.`} Earlier DSH releases need the Codex Connect build declared for them. Other combinations require verification.`;
}
...
var CodexCapabilityDiagnostics = class {
	...
	*/
	async inspect(request) {
		...
		const runtime = DSH_PLUGIN_API_PACKAGES.some((name) => versions[name] !== null && (!isSupportedDshPluginApiVersion(versions[name]) || versions[name] !== dshVersion)) || compatibility.status === "incompatible" || compatibility.status === "unverified" ? result("rejected", "declared-version-mismatch", declaredPairAction(compatibility)) : missing || compatibility.status === "unknown" ? result("unknown", "version-metadata-unavailable", "Run this command from the plugin installation in the intended profile.") : result("supported", "declared-host-versions-match", "Host package versions satisfy the declared requirements; this is not a live profile or browser compatibility test.");
